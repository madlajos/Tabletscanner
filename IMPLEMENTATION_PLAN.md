...
| D3 | **Confirmed:** USB identity is `0483:5740`; approved firmware reports the mapping/interlock/recoverable-homing marker `TS-LIGHT-V3-P0F0-P1F1-P2HE0-P3HE1-LOCK-RHOME` through `M115` and acknowledges with `ok`. | A generic Marlin identity cannot prove compatible lamp routing, firmware-level mutual exclusion, or non-fatal homing failure behavior. | Keep 115200/USB discovery and reject real controllers whose `M115` response lacks the exact marker. |
