...
import os
import json
from pipeline_models import PipelineDocument, StepInstance
...
# 2. Define steps
steps = [
    StepInstance(name="load_image", params={"source": img_path}, order=0),
    StepInstance(name="mask_rect_roi", params={
        "roi_type": "rect",
        "roi_x": 50,
        "roi_y": 50,
        "roi_width": 100,
    ...
    StepInstance(name="save_images", params={"output_dir": "test_output_v5"}, order=2)
]
...
try:
    ...
    if result.success and result.data and "images" in result.data:
        ...
        # Usually it takes result.data["images"] and saves them
        print(f"Images to save count: {len(result.data['images'])}")
    else:
        print(f"Pipeline failed. Result: {result.success}")
