...
from proc_elements import (
    ...
    automatic_rgb_composite as _pe_automatic_rgb_composite,
    focus_stack as _pe_focus_stack,
    ...
    resize_to_reference as _pe_resize_to_reference,
    ...
    align_images as _pe_align_images,
)
...
# ---------------------------------------------------------------------------
# 2/b. Pseudo image from loaded images
# ---------------------------------------------------------------------------
# Choices are generated by the inspector from loaded images. A static options
# list would reject or clamp valid references to subsequent images.
...
_pseudo_image_def = StepDefinition(
    ...
    params=[
        ParamSchema(name="blue_source", label="Kimeneti kék csatorna", type="enum",
                    default="1-B"),
        ParamSchema(name="blue_scale_percent", label="Kék réteg nagyítása (%)", type="float",
                    default=100.0, min=10.0, max=500.0, step=1.0),
        ParamSchema(name="blue_multiplier", label="Kék réteg intenzitásszorzója", type="float",
                    default=1.0, min=0.0, max=10.0, step=0.05),
        ParamSchema(name="green_source", label="Kimeneti zöld csatorna", type="enum",
                    default="1-G"),
        ParamSchema(name="green_scale_percent", label="Zöld réteg nagyítása (%)", type="float",
                    default=100.0, min=10.0, max=500.0, step=1.0),
        ParamSchema(name="green_multiplier", label="Zöld réteg intenzitásszorzója", type="float",
                    default=1.0, min=0.0, max=10.0, step=0.05),
        ParamSchema(name="red_source", label="Kimeneti piros csatorna", type="enum",
                    default="1-R"),
        ParamSchema(name="red_scale_percent", label="Piros réteg nagyítása (%)", type="float",
                    default=100.0, min=10.0, max=500.0, step=1.0),
        ParamSchema(name="red_multiplier", label="Piros réteg intenzitásszorzója", type="float",
                    default=1.0, min=0.0, max=10.0, step=0.05),
...
def _exec_pseudo_image(data: dict, params: dict) -> dict:
    return _pe_create_pseudo_image(
        ...
        blue_scale_percent=params.get("blue_scale_percent", 100.0),
        green_scale_percent=params.get("green_scale_percent", 100.0),
        red_scale_percent=params.get("red_scale_percent", 100.0),
        blue_multiplier=params.get("blue_multiplier", 1.0),
        green_multiplier=params.get("green_multiplier", 1.0),
        red_multiplier=params.get("red_multiplier", 1.0),
...
# ---------------------------------------------------------------------------
# 2/c. Automatic RGB composite from three filter captures
# ---------------------------------------------------------------------------
_automatic_rgb_composite_def = StepDefinition(
    id="automatic_rgb_composite",
    name="Automatikus RGB csatornaillesztés",
    category="adjustment",
    description="Három szűrős felvételből skálaillesztett, automatikusan fehéregyensúlyozott BGR képet készít.",
    icon="auto_fix_high",
    input_type=DataType.IMAGE,
    output_type=DataType.IMAGE,
    params=[
        ParamSchema(name="blue_image", label="Kék forráskép sorszáma", type="int", default=1, min=1, max=100, step=1),
        ParamSchema(name="green_image", label="Zöld forráskép sorszáma", type="int", default=2, min=1, max=100, step=1),
        ParamSchema(name="red_image", label="Piros forráskép sorszáma", type="int", default=3, min=1, max=100, step=1),
        ParamSchema(name="min_scale", label="Legkisebb skála", type="float", default=1.0, min=0.5, max=2.0, step=0.005),
        ParamSchema(name="max_scale", label="Legnagyobb skála", type="float", default=1.3, min=0.5, max=2.0, step=0.005),
        ParamSchema(name="search_size", label="Keresési képméret", type="int", default=700, min=100, max=2000, step=50),
        ParamSchema(name="white_balance_strength", label="Fehéregyensúly erőssége", type="float", default=0.65, min=0.0, max=1.0, step=0.05),
        ParamSchema(name="target_luminance", label="Célfényesség", type="float", default=50.0, min=1.0, max=254.0, step=1.0),
        ParamSchema(name="apply_gamma", label="Automatikus gamma", type="bool", default=True),
        ParamSchema(name="final_blur", label="Végső simítás kernel", type="int", default=3, min=1, max=15, step=2, odd_only=True),
    ],
    side_output_types={"automatic_rgb_signal_mask": "MASK", "automatic_rgb_metrics": "SCALAR"},
)


def _exec_automatic_rgb_composite(data: dict, params: dict) -> dict:
    return _pe_automatic_rgb_composite(
        data,
        blue_image=params.get("blue_image", 1),
        green_image=params.get("green_image", 2),
        red_image=params.get("red_image", 3),
        min_scale=float(params.get("min_scale", 1.0)),
        max_scale=float(params.get("max_scale", 1.3)),
        search_size=int(params.get("search_size", 700)),
        white_balance_strength=float(params.get("white_balance_strength", 0.65)),
        target_luminance=float(params.get("target_luminance", 50.0)),
        apply_gamma=bool(params.get("apply_gamma", True)),
        final_blur=int(params.get("final_blur", 3)),
    )


_register(_automatic_rgb_composite_def, _exec_automatic_rgb_composite)


# ---------------------------------------------------------------------------
# 2/d. Focus stacking from a focus bracket
# ---------------------------------------------------------------------------
_focus_stack_def = StepDefinition(
    id="focus_stack",
    name="Fókuszsorozat egyesítése",
    category="adjustment",
    description="Több fókusztávolságon készült, azonos méretű képből egy végig éles képet készít.",
    icon="filter_center_focus",
    input_type=DataType.IMAGE,
    output_type=DataType.IMAGE,
    params=[
        ParamSchema(name="focus_radius", label="Élességvizsgálat sugara", type="int",
                    default=9, min=1, max=51, step=2, odd_only=True),
        ParamSchema(name="blend_radius", label="Átmenetek simítása", type="int",
                    default=7, min=1, max=51, step=2, odd_only=True),
        ParamSchema(name="align_images", label="Képek automatikus illesztése", type="bool", default=True),
        ParamSchema(name="alignment_iterations", label="Illesztési iterációk", type="int",
                    default=80, min=10, max=500, step=10),
    ],
    side_output_types={"focus_index_map": "GRAYSCALE", "focus_stack_metrics": "SCALAR"},
)


def _exec_focus_stack(data: dict, params: dict) -> dict:
    return _pe_focus_stack(
        data,
        focus_radius=int(params.get("focus_radius", 9)),
        blend_radius=int(params.get("blend_radius", 7)),
        align_images=bool(params.get("align_images", True)),
        alignment_iterations=int(params.get("alignment_iterations", 80)),
    )


_register(_focus_stack_def, _exec_focus_stack)
...
_apply_threshold_def = StepDefinition(
    ...
    name="Küszöbölés",
    category="filter",
    description="Küszöbölés szürkeárnyalatos vagy színes képen, választható RGB-csatorna alapján.",
    icon="tonality",
    input_type=DataType.IMAGE,
    ...
    params=[
        ...
        ParamSchema(name="channel", label="Küszöbölési csatorna", type="enum",
                    default="GRAY", options=["GRAY", "R", "G", "B"],
                    description="A küszöbdöntéshez használt csatorna; RGB esetén a művelet a teljes képre hat."),
...
def _exec_apply_threshold(data: dict, params: dict) -> dict:
    ...
    channel = params.get("channel", "GRAY")
    return _pe_apply_threshold(data, thresh=thresh, maxval=maxval, mode=mode, channel=channel)
...
_calculate_histograms_def = StepDefinition(
    ...
    params=[
        ...
        ParamSchema(name="display_mode", label="Megjelenítés", type="enum",
                    default="per_image", options=["per_image", "pooled", "grouped"], required=False),
        ParamSchema(name="group_labels", label="CSV-csoportok", type="string",
                    default="[]", required=False),
        ParamSchema(name="chart_enabled", label="Ábrázolás diagramon", type="bool",
                    default=True, required=False),
    ],
    side_output_types={"histograms": "HISTOGRAM", "histogram_summary": "HISTOGRAM"},
)
...
def _exec_calculate_histograms(data: dict, params: dict) -> dict:
    ...
    return _pe_calculate_histograms(data, bins=bins, hist_range=hist_range,
        display_mode=params.get('display_mode', 'per_image'), group_labels=params.get('group_labels', '[]'))
...
_calculate_intensity_stats_def = StepDefinition(
    ...
    params=[
        ...
        ParamSchema(name="group_labels", label="CSV-csoportok", type="string",
                    default="[]", required=False,
                    description="Képenként egy csoportnév, a betöltési sorrendben; JSON-lista."),
        ParamSchema(name="chart_enabled", label="Ábrázolás diagramon", type="bool",
                    default=False, required=False),
        ParamSchema(name="chart_metric", label="Diagram Y tengelye", type="string",
                    default="mean", required=False),
    ],
    side_output_types={"intensity_stats": "SCALAR", "intensity_summary": "SCALAR"},
)
...
# Geometric alignment against another load-image branch.
_manual_image_alignment_def = StepDefinition(
    id="manual_image_alignment", name="Kézi képillesztés", category="adjustment",
    description="A fő ág képét eltolja, elforgatja és átméretezi a referenciaág képéhez.",
    icon="open_with", input_type=DataType.IMAGE, output_type=DataType.IMAGE,
    params=[
        ParamSchema(name="reference_branch", label="Referencia-képág", type="enum", default="auto", options=None),
        ParamSchema(name="offset_x", label="X eltolás (px)", type="float", default=0.0, min=-10000.0, max=10000.0, step=1.0),
        ParamSchema(name="offset_y", label="Y eltolás (px)", type="float", default=0.0, min=-10000.0, max=10000.0, step=1.0),
        ParamSchema(name="angle", label="Forgatás (fok)", type="float", default=0.0, min=-180.0, max=180.0, step=0.1),
        ParamSchema(name="scale", label="Méretarány", type="float", default=1.0, min=0.01, max=10.0, step=0.01),
        ParamSchema(name="reference_opacity", label="Referencia áttetszősége", type="float", default=0.5, min=0.0, max=1.0, step=0.05,
                    description="Csak az előnézetben: 0 a mozgatott kép, 1 a referencia."),
        ParamSchema(name="border_value", label="Háttér értéke", type="int", default=0, min=0, max=255, step=1),
    ], side_output_types={"image_alignment_transforms": "SCALAR"},
)


def _exec_manual_image_alignment(data: dict, params: dict) -> dict:
    return _pe_align_images(
        data, params.get("_alignment_reference_images", []), automatic=False,
        angle=float(params.get("angle", 0.0)), scale=float(params.get("scale", 1.0)),
        offset_x=float(params.get("offset_x", 0.0)), offset_y=float(params.get("offset_y", 0.0)),
        border_value=int(params.get("border_value", 0)),
        reference_opacity=float(params.get("reference_opacity", 0.5)),
    )


_register(_manual_image_alignment_def, _exec_manual_image_alignment)


_automatic_image_alignment_def = StepDefinition(
    id="automatic_image_alignment", name="Automatikus objektumillesztés", category="adjustment",
    description="Az objektum körvonala és képi részletei alapján automatikusan a referenciaághoz illeszti a képet.",
    icon="center_focus_strong", input_type=DataType.IMAGE, output_type=DataType.IMAGE,
    params=[
        ParamSchema(name="reference_branch", label="Referencia-képág", type="enum", default="auto", options=None),
        ParamSchema(name="allow_scale", label="Átméretezés engedélyezése", type="bool", default=True),
        ParamSchema(name="refine", label="Pontosítás képi részletekkel", type="bool", default=True),
        ParamSchema(name="border_value", label="Háttér értéke", type="int", default=0, min=0, max=255, step=1),
    ], side_output_types={"image_alignment_transforms": "SCALAR"},
)


def _exec_automatic_image_alignment(data: dict, params: dict) -> dict:
    return _pe_align_images(
        data, params.get("_alignment_reference_images", []), automatic=True,
        allow_scale=bool(params.get("allow_scale", True)), refine=bool(params.get("refine", True)),
        border_value=int(params.get("border_value", 0)),
    )


_register(_automatic_image_alignment_def, _exec_automatic_image_alignment)
...
# ---------------------------------------------------------------------------
# Resize to the pixel dimensions of an image on another branch
# ---------------------------------------------------------------------------
_resize_to_reference_def = StepDefinition(
    id="resize_to_reference",
    name="Méretezés referenciaághoz",
    category="adjustment",
    description="A saját ág képeit egy kiválasztott másik képág képeinek pixelméretére nagyítja vagy kicsinyíti.",
    icon="compare_arrows",
    input_type=DataType.IMAGE,
    output_type=DataType.IMAGE,
    params=[
        ParamSchema(name="reference_branch", label="Referencia-képág", type="enum",
                    default="auto", options=None,
                    description="A cél szélességet és magasságot adó képág. Automatikusan az előző képág használható."),
        ParamSchema(name="scale_percent", label="Nagyítás (%)", type="float",
                    default=100.0, min=1.0, max=500.0, step=1.0,
                    description="A referencia méretéhez igazítás után középpontosan nagyítja vagy kicsinyíti a képet."),
        ParamSchema(name="show_image", label="Átméretezett kép mutatása", type="bool", default=True,
                    description="Az aktuális ág átméretezett képét megjeleníti az előnézetben."),
        ParamSchema(name="show_reference", label="Referencia mutatása", type="bool", default=True,
                    description="A kiválasztott referenciaág képét rávetíti az előnézetre."),
        ParamSchema(name="reference_opacity", label="Referencia átlátszósága", type="float",
                    default=0.5, min=0.0, max=1.0, step=0.05,
                    description="0-nál az átméretezett kép, 1-nél a referencia dominál."),
        ParamSchema(name="interpolation", label="Interpoláció", type="enum",
                    default="linear", options=["nearest", "linear", "cubic", "area", "lanczos"],
                    description="A nagyítás vagy kicsinyítés képpontszámítási módja."),
    ],
)


def _exec_resize_to_reference(data: dict, params: dict) -> dict:
    return _pe_resize_to_reference(
        data,
        params.get("_alignment_reference_images", []),
        scale_percent=float(params.get("scale_percent", 100.0)),
        interpolation=params.get("interpolation", "linear"),
        show_image=bool(params.get("show_image", True)),
        show_reference=bool(params.get("show_reference", True)),
        reference_opacity=float(params.get("reference_opacity", 0.5)),
    )


_register(_resize_to_reference_def, _exec_resize_to_reference)


# ---------------------------------------------------------------------------
# Resize Images  (resize_img.py)
# ---------------------------------------------------------------------------
...
_detect_particles_def = StepDefinition(
    ...
    output_type=DataType.MASK,
    params=[
        ParamSchema(name="particle_polarity", label="Szemcse polaritása", type="enum",
                    default="dark", options=["dark", "bright"],
                    description="Sötét vagy világos szemcsék keresése a háttérhez képest"),
        ParamSchema(name="segmentation_threshold", label="Küszöbérték", type="int",
                    default=0, min=0, max=255, step=1,
                    description="0 = automatikus Otsu-küszöb; 1-255 = kézi küszöbérték"),
        ParamSchema(name="exclude_mask_edge", label="Maszk szélét érintők kihagyása", type="bool",
                    default=True,
                    description="A maszk határához érő szemcsék nem kerülnek a szűrt eredménybe és nem rajzolódnak ki"),
...
def _exec_detect_particles(data: dict, params: dict) -> dict:
    ...
    return _pe_detect_particles(
        data,
        particle_polarity=params.get("particle_polarity", "dark"),
        segmentation_threshold=int(params.get("segmentation_threshold", 0)),
        exclude_mask_edge=bool(params.get("exclude_mask_edge", True)),
        filters=filters,
        draw=draw,
...
_detect_circles_def = StepDefinition(
    ...
    output_type=DataType.IMAGE,
    params=[
        ParamSchema(name="detection_channel", label="Keresési csatorna", type="enum",
                    default="GRAY", options=["GRAY", "R", "G", "B"],
                    description="Csak a körkereséshez használt csatorna. A jelölés és a maszk az eredeti képre kerül. Szürkeárnyalatos bemenetnél minden választás ugyanazt a képet használja."),
        ...
        ParamSchema(name="radius_adjustment_percent", label="Sugár mérete (%)", type="float",
                    default=100.0, min=1.0, max=500.0, step=1.0,
                    description="A detektált sugár százaléka. 100% = eredeti sugár, 98% = 2%-kal kisebb, 110% = 10%-kal nagyobb kör."),
        ParamSchema(name="apply_mask", label="Alkalmaz maszkként", type="bool",
                    default=False,
...
def _exec_detect_circles(data: dict, params: dict) -> dict:
    ...
    radius_adjustment_percent = float(params.get("radius_adjustment_percent", 100.0))
    ...
    return _pe_detect_circles(
        data,
        detection_channel=params.get("detection_channel", "GRAY"),
        dp=dp,
        min_dist=min_dist,
        ...
        radius_adjustment_percent=radius_adjustment_percent,
...
_characterize_particles_def = StepDefinition(
    ...
    params=[
        ...
        ParamSchema(name="size_metric", label="Szemcseméret alapja", type="enum",
                    default="equivalent_diameter_px",
                    options=["equivalent_diameter_px", "area_px", "perimeter_px", "bbox_w_px", "bbox_h_px"],
                    description="Az eloszlás vízszintes tengelyén használt szemcseméret."),
        ParamSchema(name="distribution_mode", label="Képek kezelése", type="enum",
                    default="pooled", options=["pooled", "overlay", "per_image"],
                    description="Együtt számított, közös diagramon összehasonlított vagy aktuális képenkénti eloszlás."),
        ParamSchema(name="bin_count", label="Osztályközök száma", type="int",
                    default=30, min=5, max=100, step=1,
                    description="A szám- és térfogatalapú hisztogram osztályközeinek száma."),
        ParamSchema(name="output_unit", label="Mértékegység", type="enum",
                    default="px", options=["px", "mm", "um"],
                    description="Fizikai egységnél adja meg az ismert hossz pixelszámát és méretét mikrométerben."),
        ParamSchema(name="calibration_pixels", label="Kalibráció: mért hossz (pixel)", type="float",
                    default=0.0, min=0.0, max=1000000.0, step=0.1,
                    description="Az ismert hossz pixelszáma az eredeti, átméretezés előtti képen."),
        ParamSchema(name="calibration_length_um", label="Kalibráció: ismert hossz (µm)", type="float",
                    default=0.0, min=0.0, max=1000000.0, step=0.1,
                    description="A mért szakasz valós hossza mikrométerben. µm/pixel = ismert hossz / pixelszám. Mindkét mezőt töltse ki."),
        ParamSchema(name="pixels_per_mm", label="Kalibráció (pixel/mm)", type="float",
                    default=0.0, min=0.0, max=1000000.0, step=0.1,
                    description="Az eredeti, átméretezés előtti kép felbontása pixel/mm-ben."),
        ParamSchema(name="smooth_distribution", label="Eloszlás simítása", type="bool",
                    default=False,
                    description="Hárompontos súlyozott simítást alkalmaz a diagramokon. A Dv értékeket nem módosítja."),
    ],
    side_output_types={"particle_table": "SCALAR", "particle_size_distribution": "SCALAR"},
...
def _exec_characterize_particles(data: dict, params: dict) -> dict:
    ...
    return _pe_characterize_particles(
        ...
        size_metric=params.get("size_metric", "equivalent_diameter_px"),
        distribution_mode=params.get("distribution_mode", "pooled"),
        bin_count=int(params.get("bin_count", 30)),
        output_unit=params.get("output_unit", "px"),
        pixels_per_mm=params.get("pixels_per_mm", 0.0),
        calibration_pixels=params.get("calibration_pixels", 0.0),
        calibration_length_um=params.get("calibration_length_um", 0.0),
        smooth_distribution=bool(params.get("smooth_distribution", False)),
    )
...
_color_thresh_def = StepDefinition(
    ...
    category="adjustment",
    description="Szín alapú küszöbölés közvetlenül színes képen, külön konverziós lépés nélkül.",
    icon="palette",
    input_type=DataType.IMAGE,
    output_type=DataType.MASK,
    params=[
        ParamSchema(name="space", label="Színtér", type="enum",
                    default="AUTO", required=False, options=["AUTO", "HSV", "BGR", "LAB", "GRAY"],
                    description="AUTO: korábbi színtér-konverzió beállítása, ennek hiányában HSV. Külön konverziós lépés nem szükséges."),
        ...
        ParamSchema(name="output_mode", label="Kimenet", type="enum",
                    default="mask", options=["mask", "applied"],
                    description="Csak a maszk, vagy a maszkkal kivágott eredeti kép jelenjen meg."),
        ParamSchema(name="background_color", label="Háttér színe", type="string",
                    default="#000000",
                    description="A maszk 0 értékű területeinek színe."),
...
def _exec_color_thresh(data: dict, params: dict) -> dict:
    ...
    space = params.get("space", "AUTO")
    
    if space == "AUTO":
        space = data.get("meta", {}).get("select_channel", {}).get("space", "HSV")
    ...
    if space in channel_mapping:
        for ch_name, min_key, max_key in channel_mapping[space]:
            ...
            max_val = int(params.get(max_key, 179 if ch_name == "H" else 255))
            thresholds[ch_name] = (min_val, max_val)
    else:
        data["error"] = "E2201"
        return data
    
    result = _pe_color_threshold(
        data, space=space, thresholds=thresholds, invert=invert,
...
_cluster_map_def = StepDefinition(
    ...
    params=[
        ...
        ParamSchema(name="overlay_opacity", label="K-közép overlay átlátszósága", type="float",
                    default=1.0, min=0.0, max=1.0, step=0.05,
                    description="Csak az előnézeti K-közép overlay átlátszóságát állítja."),
        ParamSchema(name="show_reference_montage", label="Referenciatérképek montázsa", type="bool", default=False,
                    description="A jobb oldalon az elfogadott referenciaklaszterek térképeit mutatja."),
